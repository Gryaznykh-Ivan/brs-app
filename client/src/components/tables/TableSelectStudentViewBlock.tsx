...
import { useGetStudentTableNamesQuery } from '../../services/tableService';
import { ITableSelectRequest } from '../../types/api';

export default function TableSelectStudentViewBlock() {
    const navigate = useNavigate()
    const { subjectId = "" } = useParams()
    const { data } = useGetStudentTableNamesQuery({ subjectId })
    ...
    const onOpen = () => {
        if (tableData.id !== "") {
            navigate(`/table/view/${ tableData.id }`)
        } else {
